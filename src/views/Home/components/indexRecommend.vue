  ...
  <section class="index-recommend">
    ...
    <div class="recommend-title">
      <div class="title-group">
        <img class="title-icon" v-lazy="item.icon" />
    ...
    <div v-if="item.comicsviewtype === 1" class="recommend-type-1">
      <div
        class="item"
        v-for="childlist in item.comicslist"
        :key="childlist.bigbook_id"
      >
        <img
          class="item-pic"
          v-lazy="childlist.extension && JSON.parse(childlist.extension).xsyzfx"
        />
        <p class="item-name font-28">{{ childlist.bigbook_name }}</p>
        <p class="item-text font-24">
          {{
            childlist.extension &&
            JSON.parse(childlist.extension).recommendwords
          }}
        </p>
      </div>
    </div>
    <div v-if="item.comicsviewtype === 5" class="recommend-type-5">
      <div
        ...
        :key="childlist.bigbook_id"
      >
        <img class="item-pic" v-lazy="childlist.coverurl" />
        <p class="item-name font-28">{{ childlist.bigbook_name }}</p>
        <p class="item-text font-24">{{ childlist.key_name }}</p>
      </div>
    </div>
